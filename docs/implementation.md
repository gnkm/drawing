...
- [x] 削除: `src/app/pages/`（テンプレートのデモページ）
...
| 順 | ファイル | 内容 |
|----|----------|------|
| 1 | `src/components/drawing/result-item.tsx` | `result` を表示 |
| 2 | `src/components/drawing/results-section.tsx` | 空状態 + モック `results` |
| 3 | `src/components/drawing/kuji-card.tsx` | `kuji` を表示。削除ボタンは配置のみ |
| 4 | `src/components/drawing/box-section.tsx` | 空状態 + モック `box` |
| 5 | `src/components/drawing/add-kuji-form.tsx` | 見た目のみ（まだ controlled にしない） |
| 6 | `src/components/drawing/draw-button.tsx` / `clear-button.tsx` | 見た目 + `disabled` を props で受け取る |
| 7 | `src/components/drawing/control-panel.tsx` | 5〜6 を配置 |
| 8 | `src/components/drawing/drawing-layout.tsx` | org.md のレイアウト（`md:` で 2 カラム） |
| 9 | `src/app.tsx` | モックの `box` / `results` を定数で渡し、レイアウト全体を表示 |

- [x] ブラウザでレイアウト・空状態・カード表示を確認する
- [x] スタイルは `@/ui` を再利用する
...
- [ ] `src/hooks/use-drawing.ts` — reducer をラップ
...
- [ ] `src/app.test.tsx`（または主要コンポーネントのテスト）
  - 追加 → 削除 → 抽選 → クリアの一連フロー
